if (window.location.href.includes("https://scratch.mit.edu/discuss")) {
  if (getCookie("forum") === undefined) {
    document.cookie = `forum=${text2Binary(
      "No draft saved. Start writing and you can save a draft if necessary."
  ...
  if (getCookie("ST Features").includes("forum-draft")) {
    ...
    function setCodeForText() {
      ...
      thebutton.onclick = function () {
        document.querySelector("#id_body").value = binaryToText(
          getCookie("forum")
        );
      };
      ...
      document
        .querySelector("#reply > div.box-content")
        .appendChild(thebutton2);
    }
  }
...
if (getCookie("ST Features").includes("new-forums-page")) {
  if (
    window.location.href.includes("https://scratch.mit.edu/discuss/m/topic/")
  ) {
    window.location.href = window.location.href.replace(
      "https://scratch.mit.edu/discuss/m/topic/",
...
document.querySelectorAll("div").forEach(function (item) {
  if (
    item.textContent.includes("save code") ||
    item.textContent.includes("level code")
  ) {
    idk.push("found");
  }
});
if (idk.includes("found")) {
  ...
  var thingy = document
    .querySelector("div.confirm-email.banner")
    .cloneNode(true);
  thingy.querySelector("span").textContent =
    "Save your game codes with Zedexi instead of posting it in a game code topic to help Scratchers in need. Find out how to help ";
  document.querySelector("div#pagewrapper").prepend(thingy);
  document
    .querySelector("div#pagewrapper")
    .prepend(document.querySelector("div#topnav"));
  thingy.style.display = "";
  thingy.dataset.cue = "";
  var thingy2 = document.createElement("a");
  thingy2.textContent = "here.";
  thingy2.href =
    "https://scratch.mit.edu/discuss/topic/596390/?page=1#post-6209353";
...
if (
  window.location.href
    .toLowerCase()
    .includes("https://scratch.mit.edu/discuss/topic/")
) {
  var idk = [];
  document.querySelectorAll("div").forEach(function (item) {
    if (
      item.textContent.toLowerCase().includes("save code") ||
      item.textContent.toLowerCase().includes("level code") ||
      item.textContent.toLowerCase().includes("player code") ||
      item.textContent.toLowerCase().includes("game code")
    ) {
      console.log("found");
      idk.push("found");
    }
