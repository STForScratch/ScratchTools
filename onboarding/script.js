async function start() {
  ...
  p.textContent =
    "There are an additional 100+ features for you to choose from. You can change your settings at any time from the extension popup.";
...
async function messagesPage() {
  ...
  h1.textContent = chrome.runtime.getManifest().version_name.endsWith("-beta")
    ? "You're using the beta."
    : "Thank you.";
  h1.className = "big";
  var h3 = document.createElement("h2");
  h3.style.fontWeight = "400";
  h3.textContent = chrome.runtime.getManifest().version_name.endsWith("-beta")
    ? 'If you find any bugs, just open up the full settings page, click on the "Additional Settings" button, and click "Report a Bug".'
    : "Dozens of developers have worked hard to make hundreds of features on ScratchTools, which is used by over 6,000 people worldwide.";
  center.appendChild(h1);
  center.appendChild(h3);
  var button = document.createElement("button");
  button.textContent = "Get Started";
  button.onclick = async function () {
    window.location.href = window.user
      ? "https://scratch.mit.edu/users/" + window.user + "/"
...
async function themePage() {
  ...
  light.className =
    ...
    (currentTheme === "light" ? "theme-select" : "theme-noselect");
  light.id = "light";
  ...
  dark.className =
    "theme-circle theme-dark " +
    (currentTheme === "dark" ? "theme-select" : "theme-noselect");
  dark.id = "dark";
