function removeAdvertisements() {
  document.querySelectorAll(".comment-bubble").forEach(function (el3) {
    ...
    if (
      !el.parentNode.parentNode.parentNode.parentNode.className.includes(
        "scratchtoolsAd"
      )
    ) {
      if (el.querySelector("a") !== null) {
        ...
        el.querySelectorAll("a").forEach(function (el2) {
          if (
            el2.href
              .replaceAll(" ", "")
              .startsWith("https://scratch.mit.edu/projects/") ||
            el2.href
              .replaceAll(" ", "")
        ...
        if (
          el.innerText.toLowerCase().replaceAll(" ", "").includes("followme") ||
          ...
          el.innerText
            ...
            .replaceAll("them", "")
            .includes("checkout") ||
          (el.innerText.toLowerCase().replaceAll(" ", "").includes("lookat") &&
            el.querySelectorAll("a").length > 0) ||
          el.textContent.length - allLinks.length < allLinks.length
        ) {
  ...
  document
    .querySelectorAll(".comment > .info > .content")
    .forEach(function (el) {
      if (
        !el.parentNode.parentNode.parentNode.className.includes(
          "scratchtoolsAd"
        )
      ) {
        if (el.querySelector("a") !== null) {
          ...
          if (
            ...
            el.parentNode.parentNode.parentNode.className =
              el.parentNode.parentNode.parentNode.className + " scratchtoolsAd";
          }
        }
      }
    });
...
waitForCommentsForAdHider.observe(document.querySelector("body"), {
  attributes: true,
  childList: true,
  subtree: true,
});
