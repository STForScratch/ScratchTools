async function getStats()
{
  ...
  if(data.statistics != undefined){
    ...
    var divText = `<div id='statistics' style='text-align:center; font-size:20px; padding:2px; background-color: var(--darkWww-box, white);border-radius: 8px;border: 1px solid var(--darkWww-border-15, #d9d9d9);'>${table}${scratchstats}</div>`.replaceAll("undefined","0")
    ...
    // onclick
    spans[0].onclick = function(){click(true)}
    spans[1].onclick = function(){click(false)}
    //===============================
    };
  }
}
