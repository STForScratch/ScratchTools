async function setFeaturedProject(project, user) {
  var response = await fetch(
    "https://scratch.mit.edu/site-api/users/all/" + user + "/",
    {
      headers: {
...
if (
  ...
  if (
    document.querySelector("button.button.action-button.scratchtools") === null
  ) {
    ...
    btn.onclick = function () {
      setFeaturedProject(
        window.location.href
          ...
          .replaceAll("/", ""),
        document
          .querySelector("div.account-nav")
          .querySelector("ul")
          .querySelector("a")
          .href.replace("https://scratch.mit.edu/users/", "")
          .replaceAll("/", "")
      );
    };
  ...
  var getSpot = function (mutationList, observer) {
    ...
    for (const mutation of mutationList) {
      if (
        ...
        observer.disconnect();
        if (
          document.querySelector("button.button.action-button.scratchtools") ===
          null
        ) {
          var btn = document.createElement("button");
          btn.textContent = "Set Featured Project";
          ...
          if (document.querySelector("input.inplace-input") !== null) {
            document
              .querySelector("div.flex-row.action-buttons")
              .appendChild(btn);
          }
        }
      }
