async function checkUser() {
    var data = await (await fetch(`https://data.scratchtools.app/isonline/${window.location.pathname.split("/")[2]}/`)).json()
    if (data.scratchtools) {
        var span = document.createElement("span")
        span.className = "ste-isonline "+(data.online ? "ste-detect-online" : "ste-detect-offline")
