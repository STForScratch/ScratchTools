if (window.location.href.startsWith("https://scratch.mit.edu/mystuff")) {
  chrome.storage.sync.get("pinned", async function (obj) {
    if (
      obj !== undefined &&
      obj !== null &&
      obj.pinned !== undefined &&
      obj.pinned !== null
    ) {
      var box = addBox(
        obj.pinned.title + " • Pinned Project",
        obj.pinned.id,
...
if (window.location.href.startsWith("https://scratch.mit.edu/projects/")) {
  ...
  if (
    ...
    if (
      document.querySelector("button.button.action-button.scratchtoolspin") ===
      null
    ) {
    ...
    var getSpot = function (mutationList, observer) {
      ...
      for (const mutation of mutationList) {
        if (
          ...
          if (
            document.querySelector(
              "button.button.action-button.scratchtoolspin"
            ) === null
          ) {
            var btn = document.createElement("button");
            btn.textContent = "Pin";
