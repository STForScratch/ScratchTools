function addNoCommentBox() {
    ...
    if (document.querySelector('#comments-enabled') !== null) {
        ...
        document.querySelector('#comments-enabled').addEventListener('input', function() {
            if (document.querySelector('#comments-enabled').checked) {
                addBox()
            } else {
                if (document.querySelector('form.scratchtools.comments-on') !== null) {
                    document.querySelector('form.scratchtools.comments-on').remove()
                }
            }
        })
        }
...
div.querySelector('textarea').addEventListener('input', function() {
    div.querySelector('#chars-left').textContent = (500-(div.querySelector('textarea').value.length)).toString()
})
...
async function postComment(content) {
    var open = await fetch("https://scratch.mit.edu/site-api/comments/user/"+Scratch.INIT_DATA.LOGGED_IN_USER.model.username+"/toggle-comments/", {
        "headers": {
          "accept": "*/*",
...
var response = await fetch("https://scratch.mit.edu/site-api/comments/user/"+Scratch.INIT_DATA.LOGGED_IN_USER.model.username+"/add/", {
  "headers": {
...
var close = await fetch("https://scratch.mit.edu/site-api/comments/user/"+Scratch.INIT_DATA.LOGGED_IN_USER.model.username+"/toggle-comments/", {
    "headers": {
      "accept": "*/*",
      "accept-language": "en-US,en;q=0.9,el;q=0.8",
      "sec-ch-ua": "\" Not A;Brand\";v=\"99\", \"Chromium\";v=\"102\", \"Google Chrome\";v=\"102\"",
...
waitForNoCommentsObserver.observe(document.querySelector('body'), { attributes: true, childList: true, subtree: true });
}