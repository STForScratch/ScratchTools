...
function addOpacityRange() {
  if (document.querySelector(".scratchtoolsOpacity") === null) {
    ...
    opacity1.className =
      "button_button_u6SE2 labeled-icon-button_mod-edit-field_1bXYC scratchtoolsOpacity";
    ...
    opacity2.className =
      "button_button_u6SE2 labeled-icon-button_mod-edit-field_1bXYC";
    ...
    document
      .querySelector(".mode-tools_mode-tools_2nFfV")
      .appendChild(opacity1);
    document
      .querySelector(".mode-tools_mode-tools_2nFfV")
      .appendChild(opacity2);
    ...
    function setOpacityOfObject(element, border) {
      if (ScratchTools.Scratch.scratchPaint().selectedItems.length !== 0) {
        ...
        ScratchTools.Scratch.scratchPaint().selectedItems.forEach(function (
          el
        ) {
          if (border) {
          ...
          function setTheColor(color) {
            ...
            document
              .querySelector(
                'img.labeled-icon-button_edit-field-icon_3j-Pf[title="Flip Horizontal"]'
              )
              .parentElement.click();
            document
              .querySelector(
      ...
      if (ScratchTools.Scratch.scratchPaint().selectedItems.length === 1) {
        if (
          ScratchTools.Scratch.scratchPaint()
            .selectedItems[0].getFillColor()
            ._canvasStyle.split(",").length === 4
        ) {
          document.querySelector(".scratchtoolsFillOpacity").value = (
            Number(
              ScratchTools.Scratch.scratchPaint()
                .selectedItems[0].getFillColor()
                ._canvasStyle.split(",")[3]
                .replaceAll(", ", "")
                .replaceAll(")", "")
                .replaceAll(" ", "")
            ) * 100
        ...
        if (
          ScratchTools.Scratch.scratchPaint()
            .selectedItems[0].getStrokeColor()
            ._canvasStyle.split(",").length === 4
        ) {
          document.querySelector(".scratchtoolsBorderOpacity").value = (
            Number(
              ScratchTools.Scratch.scratchPaint()
                .selectedItems[0].getStrokeColor()
                ._canvasStyle.split(",")[3]
