...
fpsCheck.observe(document.querySelector("body"), {
  attributes: true,
  childList: true,
  subtree: true,
});
...
function setFpsCounter() {
  if (document.querySelector(".scratchtools.fps") !== null) {
    document.querySelector(".scratchtools.fps").textContent = `${Math.round(
      1000 / ScratchTools.Scratch.vm.runtime.currentStepTime
    ).toString()} FPS`;
  }
}
