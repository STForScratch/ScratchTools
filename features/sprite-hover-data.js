function getCounts() {
  ...
  document
    .querySelector("div.sprite-selector_items-wrapper_4bcOj.box_box_2jjDp")
    .childNodes.forEach(function (el) {
      if (el.firstChild.className.includes("item_is")) {
        var selectName = el.firstChild.childNodes[1].firstChild.textContent;
        document.querySelector("li#react-tabs-2").title = `${
          vm.runtime.getSpriteTargetByName(selectName).sprite.costumes_.length
        } Costumes`;
        document.querySelector("li#react-tabs-0").title = `${
          vm.runtime.getSpriteTargetByName(selectName).sprite.blocks._scripts
            .length
        } Scripts`;
        document.querySelector("li#react-tabs-4").title = `${
          vm.runtime.getSpriteTargetByName(selectName).sprite.sounds.length
        } Sounds`;
      }
    });
