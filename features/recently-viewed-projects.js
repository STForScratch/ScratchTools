...
async function getRecent() {
  ...
  var response = await fetch(
    "https://api.scratch.mit.edu/users/" +
      document
        .querySelector("ul.user-nav")
        .firstChild.firstChild.href.split("/users/")[1]
        .split("/")[0] +
      "/projects/recentlyviewed?limit=24&offset=0",
    {
      headers: {
        accept: "*/*",
  ...
  Object.keys(data).forEach(function (project) {
    ...
    pr.querySelector("a.ScratchToolsProjectTitle").textContent =
      data[project]["title"];
  ...
  function deleteButton() {
    if (
      document.querySelector("#main-content > div.grey.button") !== undefined
    ) {
