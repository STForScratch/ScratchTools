function getSpriteData() {
	try {
		...
		if (currentSpriteName.length !== 0) {
			...
			try {
				document.querySelector('#react-tabs-0 > span').textContent = `Code (${Object.keys(vm.runtime.getSpriteTargetByName(currentSpriteName).blocks._blocks).length})`
			} catch (err) {

			}
			try {
				document.querySelector('#react-tabs-2 > span').textContent = `Costumes (${vm.runtime.getSpriteTargetByName(currentSpriteName).getCostumes().length})`
			} catch (err) {

			}
			try {
				document.querySelector('#react-tabs-4 > span').textContent = `Sounds (${vm.runtime.getSpriteTargetByName(currentSpriteName).getSounds().length})`
			} catch (err) {
