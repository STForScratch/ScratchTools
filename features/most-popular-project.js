async function testStuff() {
    ...
    while (ready.length === 0) {
        var response = await fetch('https://api.scratch.mit.edu/users/'+Scratch.INIT_DATA.PROFILE.model.username+'/projects/?offset='+(offset.length*20))
        ...
        document.querySelector('div#content').insertBefore((addBox(titles[views.indexOf(maximumViews(views))], ids[views.indexOf(maximumViews(views))], views[views.indexOf(maximumViews(views))], instructions[views.indexOf(maximumViews(views))])), document.querySelector('div.box.slider-carousel-container.prevent-select'))
    }
    ...
    function addBox(title, id, views, instructions) {
        ...
        boxTitle.textContent = title+' • Most Popular Project ('+views.toString()+' views)'
